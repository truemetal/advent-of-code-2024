...
struct Position: Hashable {
    ...
    func stepInSameDirection() -> Position {
        Position(coord: coord.byMoving(direction), direction: direction)
    }

    func stepByRotatingRight() -> Position {
        Position(coord: coord, direction: direction.rotatedRight())
    }

    func stepByRotatingLeft() -> Position {
        Position(coord: coord, direction: direction.rotatedLeft())
    }

    func oppositeStep() -> Position {
        Position(coord: coord, direction: direction.rotatedRight().rotatedRight())
    }

    var description: String {
        "\(coord.x):\(coord.y)-\(direction.description)"
    }
}
